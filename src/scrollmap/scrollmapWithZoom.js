...
import dojoHas from 'dojo/has';
import popindialog from 'ebg/popindialog';
import core from 'ebg/core';
import './long-press-event';
import './core_patch_slideto';
...
export class ScrollmapWithZoom {
  ...
  set bEnableZooming(value) {
    ...
    if (this._bEnableZooming) warning_touch += _('Pinch fingers to zoom');
    this.container_div.setAttribute('warning_touch', warning_touch);
    var keysStr = this.getWheelZoomingOptionTranslated();
    this.container_div.setAttribute(
      'warning_scroll',
      dojo.string.substitute(_('Use ${keys} + Mouse Wheel to zoom the board'), { keys: keysStr }),
    );
  }
  ...
  set minHeight(value) {
    ...
    if (this.container_div) this._RepositionButtonsDiv();
  }
  get minHeight() {
  ...
  set adaptHeightCorrDivs(value) {
    ...
    for (let i = 0; i < this.adaptHeightCorrDivs.length; i++) {
      if (this._resizeHeadersObserver) this._resizeHeadersObserver.observe(this.adaptHeightCorrDivs[i]);
    }
  }
  ...
  static set bEnableKeys(value) {
    ...
    for (const inst of ScrollmapWithZoom.instances.values()) {
  ...
  constructor() {
    ...
    //     RIGHT_BOTTOM: 11,
    //   };
    const descr = Object.getOwnPropertyDescriptor(Tooltip.prototype, 'onShow');
    if (descr.writable) {
      Tooltip.prototype.onShow = ScrollmapWithZoom.onShowTooltip;
  ...
  create(
    ...
    if (typeof gameui.bUseRelPosForObjPos == 'undefined') gameui.bUseRelPosForObjPos = true;
    ...
    if (!ScrollmapWithZoom._core_patched) {
      dojo.safeMixin(gameui, new core.core_patch_slideto());
      ScrollmapWithZoom._core_patched = true;
    }
    if (surface_div) surface_div.classList.add('scrollmap_surface');
    if (onsurface_div) {
    ...
    if (!this.btns2PositionClasses) {
      if (this.btnsPositionClasses == 'btn_pos_top_right') this.btns2PositionClasses = 'btn_pos_top_left';
      if (this.btnsPositionClasses == 'btn_pos_top_left') this.btns2PositionClasses = 'btn_pos_top_right';
    }
    ...
    if (create_extra !== null) create_extra(this);
    this._pointersInit();
    ...
    if (!this._bInfoBtnVisible) this.hideInfoButton();
    this._btnToggleButtonsVisiblity = this._initButton(
      'toogle_buttons_visibility',
      this.btnToggleButtonsVisibilityHtml,
      _('Hide buttons'),
      ScrollmapWithZoom._toggleButtonsVisiblity,
    );
    ...
    if (!this._bEnableZooming) this.hideOnScreenZoomButtons();
    this.setupOnScreenResetButtons();
    this.setupEnlargeReduceButtons(
      this.incrHeightDelta,
      this.bIncrHeightKeepInPos,
      this._minHeight,
      this.bIncrHeightBtnIsShort,
      this.bIncrHeightBtnGroupedWithOthers,
    );
    if (!this._bIncrHeightBtnVisible) this.hideEnlargeReduceButtons();
    ...
    if (!this.defaultHeight) {
      ...
      if (!this.defaultHeight) this.defaultHeight = this.minHeight;
    }
    this.setDisplayHeight(this.defaultHeight);
    this.bEnableZooming = this._bEnableZooming;
    if (this.defaultZoom === null) this.defaultZoom = this.zoom;
    this.setMapZoom(this.zoom);
    if (this.startPosition) this.scrollto(-this.startPosition.x, -this.startPosition.y, 0, 0);
    else this.scrollToCenter(null, 0, 0);
    if (this._resizeObserver) this._resizeObserver.observe(this.container_div);
    ...
    aspect.after(
      ScrollmapWithZoom,
      'updateHeight',
      (new_height, incrHeightGlobalKey) => {
        if (this.incrHeightGlobalKey == incrHeightGlobalKey) this.setDisplayHeight(new_height, false);
      },
      true,
    );
    if (ScrollmapWithZoom.bEnableKeys && this.bEnableKeysArrows) {
      const warning_arrowkeys = _('press the arrow keys with ctrl key to scroll the board');
  ...
  _RepositionButtonsDiv(options = { btnsDivOnMap: undefined, btnsDivPositionOutsideMap: undefined }) {
    ...
    if (options.btnsDivOnMap != undefined) this.btnsDivOnMap = options.btnsDivOnMap;
    if (options.btnsDivPositionOutsideMap != undefined) this.btnsDivPositionOutsideMap = options.btnsDivPositionOutsideMap;
    var classList = btnsDivPositionOutsideMapPrev.split(' ');
    for (const posClass of classList) {
      if (posClass) this.container_div.classList.remove(posClass);
    }
    if (this.btnsDivOnMap) {
      this.clipped_div.appendChild(this._buttons_div);
      this.clipped_div.appendChild(this._buttons_div2);
      ...
      for (const posClass of classList) {
        if (posClass) this.container_div.classList.add(posClass);
      }
      ...
      if (this._minHeight > this.getDisplayHeight()) this.setDisplayHeight(this._minHeight);
    }
  }
  _createForm() {
    var inputs = null;
    if (!ScrollmapWithZoom._formDialog) {
      ...
      ScrollmapWithZoom._formDialog = new popindialog();
      ...
      ScrollmapWithZoom._form.onsubmit = () => {
        return false;
      };
      var keys = new Map([
      ...
      if (this.btnsDivPositionnable) {
        ...
        btnsDivPositionsSel.onmousedown = (event) => {
          event.stopPropagation();
        };
        btnsDivPositionsSel.onclick = (event) => {
          event.stopPropagation();
        };
    ...
    if (!inputs) inputs = ScrollmapWithZoom._form.elements;
  ...
  _submitForm() {
    ...
    if (addHeightNeeded) this.adaptHeight();
    if (this == ScrollmapWithZoom.instances.values().next().value) ScrollmapWithZoom._saveGameSettings();
  ...
  adaptHeight() {
    window.requestAnimationFrame(() => {
      ...
      if (!this.bAdaptHeightAuto) return;
      if (this._setupDone)
        setTimeout(() => {
          this._adaptHeightDone = true;
          this._zoomFitCalledDuringSetup = false;
        }, 3000);
      if (this._bHeightChanged) return;
      var screen_height = document.documentElement.clientHeight || document.body.clientHeight || window.innerHeight;
      ...
      for (let i = 0; i < this.adaptHeightCorrDivs.length; i++) {
        const brect = gameui.getBoundingClientRectIncludeZoom(this.adaptHeightCorrDivs[i]);
        if (brect.top + 5 >= container_pos.y + container_pos.height) other_elements_height += brect.height;
      }
      ...
      if (this.bAdaptHeightAutoCompensateChatIcon && dojo.hasClass('ebd-body', 'mobile_version')) {
        ...
        // debugger;
        if ($chatwindowavatar) {
          const brect = gameui.getBoundingClientRectIncludeZoom($chatwindowavatar);
          if (brect.height > 0) {
            const bheight = window.innerHeight - brect.top;
            if (bheight < 120) other_elements_height += bheight;
          }
        }
      }
  ...
  _onResize() {
    window.requestAnimationFrame(() => {
      if (!this._setupDone || (this.bAdaptHeightAuto && !this._adaptHeightDone)) {
        ...
        if (!this._setupDone) this._loadedSettings = this._loadSettings();
        if (!this._loadedSettings) {
          if (this._resetMode != ScrollmapWithZoom.ResetMode.ScrollAndZoomFit && this._zoomFitCalledDuringSetup) this.zoomToFit();
          if (this.startPosition) this.scrollto(-this.startPosition.x, -this.startPosition.y, 0, 0);
          else this.scrollToCenter(null, 0, 0);
        }
        if (!this._setupDone)
  ...
  _clearOldSettings() {
    const keys = Object.keys(appLocalStorage);
    let oldKeysCnt = 0;
    let oldest = null;
    let oldestKey = '';
    for (const key of keys) {
      if (key.startsWith('scrollmap')) {
        const oldSetting = JSON.parse(appLocalStorage.getItem(key));
  ...
  _loadSettings() {
    ...
    if (settingsStr != null) {
      ...
      if ((this.bAdaptHeightAuto && !this._bHeightChanged) || !this.bIncrHeightBtnVisible) this.setDisplayHeight(height);
      if (this._bHeightChanged) {
        this._enableButton(this._btnResetHeight);
      } else this._disableButton(this._btnResetHeight);
    }
    ...
    if (settingsStr != null) {
      ...
      if (settings.optionsChanged != undefined) {
        ...
        if (optionsChanged.bWheelZooming != undefined) this.zoomingOptions.bWheelZooming = optionsChanged.bWheelZooming;
        if (optionsChanged.wheelZooming != undefined) this.zoomingOptions.wheelZooming = optionsChanged.wheelZooming;
        if (optionsChanged.pinchZooming != undefined) this.zoomingOptions.pinchZooming = optionsChanged.pinchZooming;
        if (optionsChanged.bOneFingerScrolling != undefined) this.scrollingOptions.bOneFingerScrolling = optionsChanged.bOneFingerScrolling;
        ...
        if (this.btnsDivPositionnable) this._RepositionButtonsDiv(optionsChanged);
        ...
        if (adaptHeightNeeded) this.adaptHeight();
      }
    }
    return scrolled;
  }
  ...
  _getPageZoom() {
    ...
    if (typeof document.body.style.zoom !== 'undefined') {
      try {
        ...
        if (pageZoomStr !== '') pageZoom = parseFloat(pageZoomStr);
      } catch (error) {
        /* empty */
      }
    }
  ...
  _enableInteractions() {
    if (this._bEnableZooming && this.zoomingOptions.pinchZooming) this.container_div.classList.add('enable_zoom_interaction');
    if (this.bEnableScrolling) this.container_div.classList.add('enable_pan_interaction');
  ...
  _enableTooltipsAndClick() {
    //     if (SWZ.debugEn) var debugMsg = '';
    if (!this._enabledTooltips) {
      //       if (SWZ.debugEn) debugMsg += 'tooltips';
      gameui.switchDisplayTooltips(false);
      this._enabledTooltips = true;
      this._enableTooltipsAndClickTimerId = null;
    }
    if (!this._enabledClicks) {
      //       if (SWZ.debugEn) debugMsg += 'click';
      setTimeout(() => {
        this._enabledClicks = true;
        this.surface_div.removeEventListener('click', this._suppressCLickEvent_handler, this._passiveEventListener);
      }, 200);
    }
    //     if (SWZ.debugEn && debugMsg != '') SWZ.debug('_enableTooltipsAndClick enable ' + debugMsg);
  }
  _disableTooltipsAndClick(setTimer = false) {
    //     if (SWZ.debugEn) var debugMsg = '';
    if (setTimer) {
      if (this._enableTooltipsAndClickTimerId != null) clearTimeout(this._enableTooltipsAndClickTimerId);
    ...
    //     if (SWZ.debugEn && debugMsg != '') SWZ.debug('_disableTooltipsAndClick enable ' + debugMsg);
  }
  ...
  _getTouchesDist(e) {
    ...
    else return Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY);
  }
  _getTouchesMiddle(e) {
    if (e.touches.length == 1) return new DOMPoint(e.touches[0].clientX, e.touches[0].clientY);
    else return new DOMPoint((e.touches[0].clientX + e.touches[1].clientX) / 2, (e.touches[0].clientY + e.touches[1].clientY) / 2);
  }
  _handleTouch(e) {
    // var i, touch;
    if (e.type !== 'touchmove' && e.type !== 'touchstart') {
      ...
      // }
      if (e.touches.length === 0) this._touchInteracting = false;
      //       //SWZ.debug(e.touches.length);
    }
    if (
    ...
    if (e.type === 'touchstart') {
      ...
      if (e.touches.length === 1) this._touchInteracting = false;
      this._gestureStart = true;
      const touches = Array.from(e.touches);
      touches.forEach((touch) => {
        if (!this.container_div.contains(touch.target)) this._gestureStart = false;
      });
  ...
  _onPointerDown(ev) {
    // ev.preventDefault();
    if (!this.bEnableScrolling && !(this._bEnableZooming && this.zoomingOptions.pinchZooming)) return;
  ...
  _onPointerMove(ev) {
    ...
    if (this._pointers.size === 1) {
      if (!this.bEnableScrolling || ((ev.pointerType == 'touch' || ev.changedTouches) && !this._touchInteracting)) return;
      if (this._xPrev === null) [this._xPrev, this._yPrev] = this._getXYCoord(ev);
      else {
    ...
    // If two _pointers are move, check for pinch gestures
      ...
      if (this._prevDist > 0.0) {
        ...
        if (this._bEnableZooming && this.zoomingOptions.pinchZooming) this.setMapZoom(newZoom, x, y);
  ...
  _onWheel(evt) {
    if (!this._bEnableZooming) return;
    var wheelZoom = false;
  ...
  _scrollto(x, y, duration, delay, setStartPositionIfNeeded) {
    if (this._setupDone) this._scrolled = true;
    else if (setStartPositionIfNeeded) this.startPosition = { x: -x / this.zoom, y: -y / this.zoom };
  ...
  scrollToCenter(custom_css_query, duration, delay, x_extra_l = null, x_extra_r = null, y_extra_u = null, y_extra_d = null) {
    ...
    if (!custom_css_query) custom_css_query = this.centerCssQuery;
  ...
  reset(duration) {
    if (this._resetMode == ScrollmapWithZoom.ResetMode.ScrollAndZoom) this.setMapZoom(this.defaultZoom);
    if (this._resetMode == ScrollmapWithZoom.ResetMode.ScrollAndZoomFit) this.zoomToFit();
    if (this.defaultPosition) this.scrollto(-this.defaultPosition.x, -this.defaultPosition.y, duration);
    else this.scrollToCenter(null, duration);
  }
  ...
  _makeRectVisible(obj_rect, board_rect, centerOnIt = true, excl_width = 0, excl_height = 0, pos = 'topleft') {
    if (centerOnIt) {
      ...
      const delta = this._adjustToContain(board_rect, obj_rect);
      if (excl_width != 0 || excl_height != 0) {
        ...
        if (excl_rect && this._intersect(excl_rect, obj_rect)) {
          ...
          const delta1 = this._adjustToContain(board_rect1, obj_rect);
          const delta2 = this._adjustToContain(board_rect2, obj_rect);
      ...
      if (delta.x != 0 || delta.y != 0) this.scroll(delta.x, delta.y);
    }
  }
  makeObjVisible(obj, centerOnIt = false, excl_width = 0, excl_height = 0, pos = 'topleft') {
    const board_rect = gameui.getBoundingClientRectIgnoreZoom(this.clipped_div);
    const obj_rect = gameui.getBoundingClientRectIgnoreZoom(obj);
  ...
  getMapLimits(custom_css_query = null) {
    ...
    function calcMaxMin(node, top_div) {
      ...
      const left = node.offsetLeft * scaleTotal || 0;
      const width = parseFloat(s.width) * scaleTotal || node.offsetWidth * scaleTotal;
      max_x = max_x !== null ? Math.max(max_x, left + width) : left + width;
      min_x = min_x !== null ? Math.min(min_x, left) : left;
      const top = node.offsetTop * scaleTotal || 0;
      const height = parseFloat(s.height) * scaleTotal || node.offsetHeight * scaleTotal;
  ...
  zoomToFit(x_extra_l = null, x_extra_r = null, y_extra_u = null, y_extra_d = null, cover_arrows = null) {
    ...
    if (cover_arrows !== null) this._cover_arrows = cover_arrows;
    else cover_arrows = this._cover_arrows;
    const { min_x, max_x, min_y, max_y } = this.getMapLimits();
    var container_width = this.clipped_div.clientWidth;
    if (cover_arrows === false) container_width -= 2 * this._btnMoveLeft.clientWidth;
    var container_height = this.clipped_div.clientHeight;
    if (cover_arrows === false) container_height -= 2 * this._btnMoveTop.clientHeight;
    const newZoom = Math.min(
      container_width / (max_x - min_x + x_extra_l + x_extra_r),
    ...
    if (!this._setupDone) this._zoomFitCalledDuringSetup = true;
  }
  ...
  setMapZoom(zoom, x = 0, y = 0) {
    if (zoom >= this.maxZoom) {
      ...
      if (this._btnZoomMinus && this._prevZoom <= this.minZoom) this._enableButton(this._btnZoomMinus);
      if (this._btnZoomPlus && this._prevZoom >= this.maxZoom) this._enableButton(this._btnZoomPlus);
    }
    this.zoom = zoom;
    ...
    if (this.animation_div !== null) this._setScale(this.animation_div, this.zoom);
    ...
    if (this._prevZoom == this.zoom) return;
    if (this.zoomChangeHandler) this.zoomChangeHandler(this.zoom);
  ...
  _getButton(btnNames, idSuffix = '') {
    ...
    for (const i in btnNamesL) {
      const btnName = btnNamesL[i];
      var $btn = null;
      var $querydiv;
      if (idSuffix == '') $querydiv = this.container_div;
      else $querydiv = document.getElementById(this.container_div.id + idSuffix);
      if ($querydiv != null) $btn = $querydiv.querySelector('.' + this._classNameSuffix + btnName);
      //       //SWZ.debug($btn);
      //       //SWZ.debug('#' + this.container_div.id+idSuffix + ' .'+this._classNameSuffix+btnName);
      if ($btn === null) $btn = $(btnName);
      if ($btn) {
  ...
  _setButtonsVisiblity(visible, dispatch = true) {
    ...
    if (!visible && !this.btnsDivOnMap) return;
    this.container_div.querySelectorAll('.scrollmap_button_wrapper').forEach((node) => {
      if (visible && !node.classList.contains('scrollmap_icon_always_invisible')) node.classList.remove('scrollmap_btn_nodisplay');
      else if (!node.classList.contains('scrollmap_icon_always_visible')) node.classList.add('scrollmap_btn_nodisplay');
    });
  }
  ...
  _initButton(
    ...
    if (typeof destDiv === 'undefined' || destDiv != null) {
      ...
      if (typeof destDiv === 'undefined') destDiv = this._buttons_div;
  ...
  _onButtonLongPress(onLongPressedAnim, evt) {
    ...
    var _longPressAnim = (time, anim = onLongPressedAnim) => {
      anim();
      if (this._longPress) requestAnimationFrame(_longPressAnim);
    };
  ...
  _onKeyDown(e) {
    if (!ScrollmapWithZoom.bEnableKeys || ScrollmapWithZoom.count != 1) return;
    if (gameui.chatbarWindows['table_' + gameui.table_id].status == 'expanded') return;
    ...
    switch (e.key) {
      ...
      case '+':
      case '-':
        if (this._keysPressed.size == 1) {
          // only this key pressed
          e.preventDefault();
          e.stopImmediatePropagation();
          break;
        }
  ...
  _onKeyLongPress(key) {
    ...
    if (!this._keysPressed.get(key)) return false;
    this._keysPressed.get(key).timer = null;
    var _longPressAnim = (time) => {
      this._onKeyLongPressAnim(key);
      if (this._keysPressed.get(key)) requestAnimationFrame(_longPressAnim);
    };
  ...
  _onKeyLongPressAnim(key) {
    ...
    switch (key) {
      case 'ArrowLeft':
        if (this.bEnableKeysArrows && this._keysPressed.get('Control')) this.scroll(this._longPressScrollOriented, 0, 0, 0);
        break;
      case 'ArrowRight':
        if (this.bEnableKeysArrows && this._keysPressed.get('Control')) this.scroll(-this._longPressScrollOriented, 0, 0, 0);
        break;
      case 'ArrowUp':
        if (this.bEnableKeysArrows && this._keysPressed.get('Control')) this.scroll(0, this._longPressScrollOriented, 0, 0);
        break;
      case 'ArrowDown':
        if (this.bEnableKeysArrows && this._keysPressed.get('Control')) this.scroll(0, -this._longPressScrollOriented, 0, 0);
        break;
      case '+':
        if (this.bEnableKeysPlusMinus) this.changeMapZoom(this._longPressScrollOriented);
        break;
      case '-':
        if (this.bEnableKeysPlusMinus) this.changeMapZoom(-this._longPressScrollOriented);
        break;
  ...
  _onKeyUp(e) {
    if (!ScrollmapWithZoom.bEnableKeys || ScrollmapWithZoom.count != 1) return;
    setTimeout(() => {
      this.container_div.classList.remove('scrollmap_warning_arrowkeys');
    }, 3000);
    if (!this._keysPressed.get(e.key)) return;
    ...
    switch (e.key) {
      case 'ArrowLeft':
        if (this.bEnableKeysArrows && this._keysPressed.get('Control')) this._onMoveLeft(e);
        break;
      case 'ArrowRight':
        if (this.bEnableKeysArrows && this._keysPressed.get('Control')) this._onMoveRight(e);
        break;
      case 'ArrowUp':
        if (this.bEnableKeysArrows && this._keysPressed.get('Control')) this._onMoveTop(e);
        break;
      case 'ArrowDown':
        if (this.bEnableKeysArrows && this._keysPressed.get('Control')) this._onMoveDown(e);
        break;
      case '+':
        if (this.bEnableKeysPlusMinus) this._onZoomIn(e);
        break;
      case '-':
        if (this.bEnableKeysPlusMinus) this._onZoomOut(e);
        break;
      case 'Home':
        if (this.bEnableKeyHome && this._keysPressed.get('Control')) this.scrollToCenter();
        break;
      case 'End':
        if (this.bEnableKeyEnd && this._keysPressed.get('Control')) this.zoomToFit();
        break;
      default:
        handled = false;
    }
  ...
  setupOnScreenArrows(scrollDelta, bScrollDeltaAlignWithZoom = true) {
    ...
    if (this.bScrollDeltaAlignWithZoom) this._scrollDeltaAlignWithZoom = scrollDelta * this.zoom;
    else this._scrollDeltaAlignWithZoom = scrollDelta;
    if (!this._btnMoveTop) {
      this._btnMoveTop = this._initButton(
        'movetop',
        this.btnMoveTopHtml,
        _btnsMoveHelp,
        this._onMoveTop,
    ...
    if (!this._btnMoveDown) {
      this._btnMoveDown = this._initButton(
        'movedown',
        this.btnMoveDownHtml,
        _btnsMoveHelp,
        this._onMoveDown,
        () => {
          this.scroll(0, -this._longPressScrollOriented, 0, 0);
    ...
    if (!this._btnMoveLeft) {
      this._btnMoveLeft = this._initButton(
        'moveleft',
        this.btnMoveLeftHtml,
        _btnsMoveHelp,
        this._onMoveLeft,
        () => {
    ...
    if (!this._btnMoveRight) {
      this._btnMoveRight = this._initButton(
        'moveright',
        this.btnMoveRightHtml,
        _btnsMoveHelp,
        this._onMoveRight,
        () => {
          this.scroll(-this._longPressScrollOriented, 0, 0, 0);
  ...
  setupOnScreenResetButtons(resetMode = ScrollmapWithZoom.ResetMode.Scroll) {
    ...
    if (!this._btnZoomToFit)
      this._btnZoomToFit = this._initButton(this._btnZoomToFitNames, this.btnZoomToFitHtml, _('Fit map to display area'), () =>
        this.zoomToFitAndScrollToCenter(),
      );
  ...
  _setupEnlargeReduceButtons(bInsideMap, bShort = true, destDiv = null) {
    ...
    if (!this._btnIncreaseHeight)
      this._btnIncreaseHeight = this._initButton(
        this._btnIncreaseHeightNames,
        bInsideMap ? (bShort ? this.btnIncreaseHeightShortHtml : this.btnIncreaseHeightHtml) : null,
        _('Increase height'),
        this._onIncreaseDisplayHeight,
        () => {
    ...
    if (!this._btnDecreaseHeight)
      this._btnDecreaseHeight = this._initButton(
        this._btnDecreaseHeightNames,
        bInsideMap ? (bShort ? this.btnDecreaseHeightShortHtml : this.btnDecreaseHeightHtml) : null,
        _('Decrease height'),
        this._onDecreaseDisplayHeight,
        () => {
          this.changeDisplayHeight(-5);
        },
        bShort ? destDiv : null,
  ...
  setupEnlargeReduceButtons(incrHeightDelta, bIncrHeightKeepInPos = true, minHeight = null, bShort = true, bGroupedWithOthers = true) {
    ...
    // }
    this._setupEnlargeReduceButtons(true, bShort, buttonsDiv);
    this._btnResetHeight = this._initButton(
      'reset_height',
      this.btnResetHeightHtml,
      _('Reset Height'),
      this._onResetHeight,
      null,
      buttonsDiv,
    );
    this._btnMaximizeHeight = this._initButton(
      'maximize_height',
      this.btnMaximizeHeightHtml,
      _('Maximize Height'),
      this._onMaximizeHeight,
      null,
      buttonsDiv,
    );
    if (this._bMaxHeight) {
  ...
  _onResetHeight(evt) {
    this._bMaxHeight = false;
    this._bHeightChanged = false;
    if (this.bAdaptHeightAuto) this.adaptHeight();
    else this.setDisplayHeight(this.defaultHeight);
    this._disableButton(this._btnResetHeight);
    this._enableButton(this._btnMaximizeHeight);
  }
  ...
  setDisplayHeight(new_height, dispatch = true) {
    ...
    if (this.bIncrHeightKeepInPos && this._setupDone) this.board_y += (current_height - new_height) / 2;
  ...
  setupInfoButton(bConfigurableInUserPreference = false) {
    if (!this._btnInfo) {
      const $btn = this._initButton('info', this.btnInfoHtml);
      $btn.classList.add('scrollmap_icon_always_visible');
  ...
  getInfoButtonTooltip() {
    ...
    if (ScrollmapWithZoom.bEnableKeys && this.bEnableKeysArrows) {
      const keysStr = _('ctrl');
      info +=
        '<li>' +
        dojo.string.substitute(_('press the arrow keys with ${keys} (long press : continious scroll/pan).'), { keys: keysStr }) +
        '</li>';
    }
    info += '</ul>';
    if (this._bEnableZooming) {
      ...
      info += '<ul>';
      const keysStr = this.getWheelZoomingOptionTranslated();
      info += '<li>';
      info +=
        `<span style="${this.zoomingOptions.bWheelZooming ? '' : 'text-decoration: line-through;'}">` +
        dojo.string.substitute(_('use the mouse wheel with ${keys}'), { keys: keysStr }) +
        '</span>';
      info += canbemodfied;
      info += '</li>';
      info +=
        '<li>' +
        `<span style="${this.zoomingOptions.pinchZooming ? '' : 'text-decoration: line-through;'}">` +
        _('pinch fingers.') +
        '</span>' +
        canbemodfied +
        '</li>';
    ...
    if (ScrollmapWithZoom.bEnableKeys && this.bEnableKeyHome) info += '<li>' + _('press the home key with ctrl key') + '</li>';
    info += '</ul>';
    if (ScrollmapWithZoom.bEnableKeys && this.bEnableKeyEnd) info += '<BR>' + _('To fit to content : press the end key with ctrl key');
    if (this._bConfigurableInUserPreference) info += _('This is configurable in user preference.');
    info += '<BR>' + '(v ' + this.version + ')';
    info += '</div>';
    return info;
  }
  setInfoButtonTooltip() {
    if (!this._btnInfo) return;
    if (gameui != null) {
      gameui.addTooltipHtml(this._btnInfo.id, '', 10);
      gameui.tooltips[this._btnInfo.id].bForceOpening = true;
      gameui.tooltips[this._btnInfo.id].getContent = () => {
        return this.getInfoButtonTooltip();
      };
    }
  }
  setBShowMoveCursor() {
...
dojoHas.add('config-tlmSiblingOfDojo', 0, 0, 1);
