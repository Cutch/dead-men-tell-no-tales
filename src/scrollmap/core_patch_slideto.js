...
declare('ebg.core.core_patch_slideto', null, {
  constructor: function () {
    ...
    function _checkIfZoomImplemented() {
      ...
      if (tBox.x != 10) this._boundingRectIncludeZoom = tBox.width != 100;
  ...
  calcCurrentCSSZoom: function (node, cstyle) {
    ...
    let zoom = 1.0;
    if (!cstyle) cstyle = window.getComputedStyle(node);
    ...
    if (parent) zoom = zoom * this.calcCurrentCSSZoom(parent);
    return zoom;
  },

  calcScale: function (element, cstyle) {
    if (!cstyle) cstyle = window.getComputedStyle(element);
    ...
    else return scale * this.calcScale(parent);
  },
  ...
  _calcTransform: function (element, cstyle = null, clearTranslation = true) {
    ...
    if (parent !== null) {
      ...
      if (matrix === null) matrix = matrixParent;
      else if (matrixParent !== null) matrix.preMultiplySelf(matrixParent);
    }
  ...
  _calcNewLocation: function (
    ...
    if (target_x == null) {
      ...
      var delta_x = 0;
      if (matrix !== null) delta_x = matrix.transformPoint(new DOMPoint(0, -mobile_obj.offsetHeight)).x;
    ...
    var vector_abs = new DOMPoint(tgt.x - src.x, tgt.y - src.y);
    ...
    if (matrix !== null) vector = matrix.inverse().transformPoint(vector_abs);
    vector.x /= zoomCorr;
    vector.y /= zoomCorr;
    left += vector.x;
    top += vector.y;
  ...
  calcNewLocation: function (mobile_obj, target_obj, target_x, target_y, bRelPos, bMobileObjectCenter, bToTargetCenter) {
    if (typeof mobile_obj == 'string') mobile_obj = $(mobile_obj);
    if (typeof target_obj == 'string') target_obj = $(target_obj);
  ...
  placeOnObjectPos: function (mobile_obj, target_obj, target_x, target_y, bRelPos, bAlignCenters = true) {
    if (typeof bRelPos == 'undefined') bRelPos = this.bUseRelPosForObjPos;
  ...
  _slideToObject: function (mobile_obj, target_obj, target_x, target_y, bRelPos, duration, delay, bMobileObjectCenter, bToTargetCenter) {
    ...
    var [left, top, vector_x, vector_y] = this.calcNewLocation(
      mobile_obj,
      target_obj,
      target_x,
      target_y,
  ...
  slideToObjectPos: function (mobile_obj, target_obj, target_x, target_y, duration, delay, bAlignCenters) {
    return this._slideToObject(
      mobile_obj,
      target_obj,
      target_x,
      target_y,
      this.bUseRelPosForObjPos,
      duration,
      delay,
      bAlignCenters,
      bAlignCenters,
